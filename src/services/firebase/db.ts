...
export const onceGetUser = async (
  uid: string
): Promise<firebase.database.DataSnapshot> => db.doc(`/users/${uid}`).get();
...
export const onceGetHumanResource = (
  ...
  db
    .ref(`/facilities/${globalFacility}/humanResources/${hResource.id}`)
    .once("value");

export const onceGetHumanResources =
  (): Promise<firebase.database.DataSnapshot> =>
